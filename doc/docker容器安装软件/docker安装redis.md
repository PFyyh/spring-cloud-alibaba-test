...
**进入Redis数据卷所在目录，这里是/docker/redis:**

````shell
docker run -d --privileged=true -p 6379:6379 -v $pwd/conf/redis.conf:/etc/redis/redis.conf -v $pwd/data:/data --name redis redis:3.2 redis-server /etc/redis/redis.conf --appendonly yes
````
